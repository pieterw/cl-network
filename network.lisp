...
(export '(net-node id edges net-edge label node-edges weight start end
network nodes edges add-edge add-node primitives complete-edges
delete-node delete-edge deep-copy-network neighbours all-neighbours
neighbours-in neighbours-out create-and-add-edge get-sub-network))
...
(defmethod shallow-copy-node ((node net-node))
  (make-instance 'net-node
                 :id (gensym)
                 :node-edges (copy-list (node-edges node))))
...
(defclass net-edge ()
  ((id :documentation "Every edge must have a unique ID."
   ...
   (label :documentation "The label of the edge. Need not be unique."
	  ...
	  :initarg :label
	  :accessor label)
   (weight :documentation "The weight of the edge."
	  :type real
	  :initform 0.5
	  :initarg :weight
	  :accessor weight)
...
(defun make-net-edge (start label end &key (id (gensym)) (weight 0.5))
  (make-instance 'net-edge
                 :start start :label label :end end
                 :id id :weight weight))

(defmethod shallow-copy-edge ((edge net-edge))
  (make-instance 'net-edge
                 :id (gensym)
                 :label (label edge)
                 :weight (weight edge)
                 :start (start edge) ;; no copy here (use deep-copy-network)
                 :end (end edge)))

(defmethod print-object ((edge net-edge) stream)
  (pprint-logical-block (stream nil)
    (format stream "<edge (~a, ~a) ~a" 
	    (short-id (id edge)) (weight edge) (label edge))
    (call-next-method)
    (format stream ">")))
...
(defmethod deep-copy-network ((network network) &key (check-consistency t))
  (let* ((node-mapping (make-hash-table)) 
    ...
    (complete-edges copy :check-consistency check-consistency) 
    ;; The above will fill the (edges ...) slots of the nodes
    copy))
